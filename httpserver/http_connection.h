...
constexpr auto SISPOP_SENDER_SNODE_PUBKEY_HEADER = "X-Sispop-Snode-PubKey";
constexpr auto SISPOP_SNODE_SIGNATURE_HEADER = "X-Sispop-Snode-Signature";
constexpr auto SISPOP_SENDER_KEY_HEADER = "X-Sender-Public-Key";
constexpr auto SISPOP_TARGET_SNODE_KEY = "X-Target-Snode-Key";
...
class SispopdClient {
    ...
    std::string sispopd_rpc_ip_;
    const uint16_t sispopd_rpc_port_;

  public:
    SispopdClient(boost::asio::io_context& ioc, std::string ip, uint16_t port);
    void make_sispopd_request(boost::string_view method,
                            const nlohmann::json& params,
                            http_callback_t&& cb) const;
    void make_custom_sispopd_request(const std::string& daemon_ip,
                                   const uint16_t daemon_port,
                                   boost::string_view method,
                                   const nlohmann::json& params,
                                   http_callback_t&& cb) const;
    // Synchronously fetches the private key from sispopd.  Designed to be called *before* the
...
constexpr const char *error_string(SNodeError err) {
    switch (err) {
        case sispop::SNodeError::NO_ERROR:
            return "NO_ERROR";
        case sispop::SNodeError::ERROR_OTHER:
            return "ERROR_OTHER";
        case sispop::SNodeError::NO_REACH:
            return "NO_REACH";
        case sispop::SNodeError::HTTP_ERROR:
            return "HTTP_ERROR";
        default:
            return "[UNKNOWN]";
    }
...
template <>
struct formatter<sispop::SNodeError> {
    ...
    auto format(const sispop::SNodeError& err, FormatContext& ctx) {
