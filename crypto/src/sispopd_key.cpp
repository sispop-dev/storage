...
private_key_t sispopdKeyFromHex(const std::string& private_key_hex) {
    if (private_key_hex.size() != KEY_LENGTH * 2)
        throw std::runtime_error(
                "Sispopd key data is invalid: expected " + std::to_string(KEY_LENGTH) + " bytes not " +
...
private_key_ed25519_t::from_hex(const std::string& sc_hex) {
    if (sc_hex.size() != private_key_ed25519_t::LENGTH * 2)
        throw std::runtime_error("Sispopd key data is invalid: expected " +
