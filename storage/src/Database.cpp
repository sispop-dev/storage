...
bool Database::get_message_count(uint64_t& count) {
    ...
    while (true) {
        ...
        if (rc == SQLITE_BUSY) {
            ...
            SISPOP_LOG(critical, "Could not execute `count` db statement");
            break;
        }
    }
    ...
    if (rc != SQLITE_OK) {
        SISPOP_LOG(critical, "sqlite reset error: [{}], {}", rc,
                 sqlite3_errmsg(db));
        success = false;
    }
...
bool Database::store(const std::string& hash, const std::string& pubKey,
    ...
    while (true) {
        ...
        if (rc == SQLITE_BUSY) {
            ...
            SISPOP_LOG(critical, "Could not execute `store` db statement, ec: {}",
                     rc);
            break;
        }
    }
