...
static constexpr auto SISPOP_EPHEMKEY_HEADER = "X-Sispop-EphemKey";
static constexpr auto SISPOP_LONG_POLL_HEADER = "X-Sispop-Long-Poll";

static constexpr auto SISPOP_FILE_SERVER_TARGET_HEADER = "X-Sispop-File-Server-Target";
static constexpr auto SISPOP_FILE_SERVER_VERB_HEADER = "X-Sispop-File-Server-Verb";
static constexpr auto SISPOP_FILE_SERVER_HEADERS_HEADER = "X-Sispop-File-Server-Headers";
...
// ======================== Sispopd Client ========================
SispopdClient::SispopdClient(boost::asio::io_context& ioc, std::string ip, uint16_t port)
    : ioc_(ioc), sispopd_rpc_ip_(std::move(ip)), sispopd_rpc_port_(port) {}

void SispopdClient::make_sispopd_request(boost::string_view method,
                                     const nlohmann::json& params,
                                     http_callback_t&& cb) const {

    make_custom_sispopd_request(sispopd_rpc_ip_, sispopd_rpc_port_, method, params,
                              std::move(cb));
}

void SispopdClient::make_custom_sispopd_request(const std::string& daemon_ip,
    ...
    SISPOP_LOG(trace, "Making sispopd request, method: {}", method.to_string());

    make_http_request(ioc_, daemon_ip, daemon_port, req, std::move(cb));
}

static bool validateHexKey(const std::string& key, const size_t key_length = sispop::KEY_LENGTH) {
...
SispopdClient::wait_for_privkey() {
    // fetch SN private key from sispopd; do this synchronously because we can't finish startup
    // until we have it.
    sispop::private_key_t private_key;
    sispop::private_key_ed25519_t private_key_ed;
    sispop::private_key_t private_key_x;
    SISPOP_LOG(info, "Retrieving SN key from sispopd");
    boost::asio::steady_timer delay{ioc_};
    std::function<void(sispop::sn_response_t &&res)> key_fetch;
    key_fetch = [&](sispop::sn_response_t res) {
        try {
            if (res.error_code != sispop::SNodeError::NO_ERROR)
                throw std::runtime_error(sispop::error_string(res.error_code));
            else if (!res.body)
                ...
                if (!validateHexKey(legacy_privkey) || !validateHexKey(privkey_ed, private_key_ed25519_t::LENGTH) || !validateHexKey(privkey_x))
                    ...
                    private_key = sispop::sispopdKeyFromHex(legacy_privkey);
                    // TODO: check that one is derived from the other as a sanity check?
                    private_key_ed = private_key_ed25519_t::from_hex(privkey_ed);
                    private_key_x = sispop::sispopdKeyFromHex(privkey_x);
                    // run out of work, which will end the event loop
                }
            }
        } catch (const std::exception &e) {
            SISPOP_LOG(critical, "Error retrieving SN privkey from sispopd @ {}:{}: {}.  Is sispopd running?  Retrying in 5s",
                     sispopd_rpc_ip_, sispopd_rpc_port_, e.what());

            delay.expires_after(std::chrono::seconds{5});
            delay.async_wait([this, &key_fetch](const boost::system::error_code &) {
                    make_sispopd_request("get_service_node_privkey", {}, key_fetch); });
        }
    };
    make_sispopd_request("get_service_node_privkey", {}, key_fetch);
...
accept_connection(boost::asio::io_context& ioc,
    ...
    acceptor.async_accept([&](const error_code& ec, tcp::socket socket) {
        SISPOP_LOG(trace, "connection accepted");
        if (!ec) {
            ...
            if (ec == boost::system::errc::too_many_files_open) {
                SISPOP_LOG(critical, "Too many open files, aborting");
                abort();
            }
...
void run(boost::asio::io_context& ioc, const std::string& ip, uint16_t port,
    ...
    SISPOP_LOG(trace, "http server run");
...
connection_t::~connection_t() {
    ...
    if (stream_.lowest_layer().is_open()) {
        SISPOP_LOG(debug, "Client socket should be closed by this point, but "
                        "wasn't. Closing now.");
...
void connection_t::on_handshake(boost::system::error_code ec) {
    ...
    SISPOP_LOG(trace, "Open https socket: {}", sockfd);
    get_net_stats().record_socket_open(sockfd);
    if (ec) {
        SISPOP_LOG(warn, "ssl handshake failed: ec: {} ({})", ec.value(), ec.message());
        this->clean_up();
        deadline_.cancel();
        return;
    }
...
void connection_t::notify(boost::optional<const message_t&> msg) {
    ...
    if (msg) {
        SISPOP_LOG(trace, "Processing message notification: {}", msg->data);
...
void connection_t::read_request() {
    ...
    auto on_data = [self = shared_from_this()](error_code ec,
        ...
        SISPOP_LOG(trace, "on data: {} bytes", bytes_transferred);
        ...
        try {
            ...
            SISPOP_LOG(critical, "Exception caught processing a request: {}",
                     e.what());
...
bool connection_t::validate_snode_request() {
    if (!parse_header(SISPOP_SENDER_SNODE_PUBKEY_HEADER,
                      SISPOP_SNODE_SIGNATURE_HEADER)) {
        SISPOP_LOG(debug, "Missing signature headers for a Service Node request");
        return false;
    }
    const auto& signature = header_[SISPOP_SNODE_SIGNATURE_HEADER];
    const auto& public_key_b32z = header_[SISPOP_SENDER_SNODE_PUBKEY_HEADER];
    ...
    if (!service_node_.is_snode_address_known(snode_address)) {
        ...
        SISPOP_LOG(debug, "Discarding signature from unknown service node: {}",
                 public_key_b32z);
...
void connection_t::process_storage_test_req(uint64_t height,
    ...
    SISPOP_LOG(trace, "Performing storage test, attempt: {}", repetition_count_);
    ...
    if (status == MessageTestStatus::SUCCESS) {
        ...
        SISPOP_LOG(error, "Failed storage test, tried {} times.",
                 repetition_count_);
        nlohmann::json json_res;
...
void connection_t::process_blockchain_test_req(uint64_t,
    ...
    SISPOP_LOG(debug, "Performing blockchain test");
...
void connection_t::process_proxy_req() {
    ...
    if (!parse_header(SISPOP_SENDER_KEY_HEADER,
                      SISPOP_TARGET_SNODE_KEY)) {
        SISPOP_LOG(debug, "Missing headers for a proxy request");
        return;
    }

    const auto& sender_key = header_[SISPOP_SENDER_KEY_HEADER];
    const auto& target_snode_key = header_[SISPOP_TARGET_SNODE_KEY];
...
void connection_t::process_file_proxy_req() {
    ...
    SISPOP_LOG(debug, "Processing a file proxy request: we are first hop");

    const request_t& original_req = this->request_.get();

    delay_response_ = true;

    if (!parse_header(SISPOP_FILE_SERVER_TARGET_HEADER,
                      SISPOP_FILE_SERVER_VERB_HEADER,
                      SISPOP_FILE_SERVER_HEADERS_HEADER)) {
        SISPOP_LOG(error, "Missing headers for a file proxy request");
    ...
    const auto& target = header_[SISPOP_FILE_SERVER_TARGET_HEADER];
    const auto& verb_str = header_[SISPOP_FILE_SERVER_VERB_HEADER];
    const auto& headers_str = header_[SISPOP_FILE_SERVER_HEADERS_HEADER];

    SISPOP_LOG(trace, "Target: {}", target);
    SISPOP_LOG(trace, "Verb: {}", verb_str);
    SISPOP_LOG(trace, "Headers json: {}", headers_str);

    const json headers_json = json::parse(headers_str, nullptr, false);

    if (headers_json.is_discarded()) {
        SISPOP_LOG(debug, "Bad file proxy request: invalid header json");
...

    {
        ...
        if (it != original_req.end()) {
            SISPOP_LOG(trace, "Content-Type: {}", it->value().to_string());
    ...
    req->target(target);
    req->set(http::field::host, "file.sispopnet.org");
    ...
    auto cb = [this](sn_response_t res) {
        SISPOP_LOG(trace, "Successful file proxy request!");

        if (res.raw_response) {
            this->response_ = *res.raw_response;
            SISPOP_LOG(trace, "Response: {}", this->response_);
        } else {
            SISPOP_LOG(debug, "No response from file server!");
        }
...
void connection_t::process_swarm_req(boost::string_view target) {
    ...
    if (target == "/swarms/storage_test/v1") {
        ...
        response_.result(http::status::bad_request);
        SISPOP_LOG(debug, "Got storage test request");
        ...
        if (body == nlohmann::detail::value_t::discarded) {
            SISPOP_LOG(debug, "Bad snode test request: invalid json");
        ...
        try {
            ...
            response_.result(http::status::bad_request);
            SISPOP_LOG(debug, "Bad snode test request: missing fields in json");
            return;
        }

        const auto it = header_.find(SISPOP_SENDER_SNODE_PUBKEY_HEADER);
        if (it != header_.end()) {
            const std::string& tester_pk = it->second;
            this->process_storage_test_req(blk_height, tester_pk, msg_hash);
        } else {
            SISPOP_LOG(debug, "Ignoring test request, no pubkey present");
        }
    } else if (target == "/swarms/blockchain_test/v1") {
        SISPOP_LOG(debug, "Got blockchain test request");
        ...
        try {
            ...
            if (body.find("height") != body.end()) {
                ...
                SISPOP_LOG(debug, "No tester height, defaulting to {}", height);
            }
        } catch (...) {
            response_.result(http::status::bad_request);
            SISPOP_LOG(debug, "Bad snode test request: missing fields in json");
        ...
        SISPOP_LOG(debug, "Received ping_test");
        response_.result(http::status::ok);
    } else if (target == "/swarms/push/v1") {

        SISPOP_LOG(trace, "swarms/push");
        ...
        SISPOP_LOG(debug, "Processing proxy request: we are the destination node");

#ifdef INTEGRATION_TEST
        // print_headers(req);
#endif

        const auto it = req.find(SISPOP_SENDER_KEY_HEADER);
        if (it != req.end()) {
            ...
            try {
                ...
                SISPOP_LOG(error, "JSON parsing error: {}", e.what());
            }
        }
...
void connection_t::process_request() {
    ...
    SISPOP_LOG(trace, "connection_t::process_request");
    ...
    case http::verb::post: {
        ...
        if (target == "/storage_rpc/v1") {
            ...
            SISPOP_LOG(trace, "POST /storage_rpc/v1");
            ...
            ioc_.post([]() {
                SISPOP_LOG(warn, "Sleeping for some time...");
            ...
            SISPOP_LOG(debug, "unknown target for POST: {}", target.to_string());
    ...
    case http::verb::get:
        ...
        if (target == "/get_stats/v1") {
            ...
            this->body_stream_ << fmt::format("unknown target for GET: {}",
                                              target.to_string());
            SISPOP_LOG(debug, "unknown target for GET: {}", target.to_string());
            response_.result(http::status::not_found);
        }
        break;
    default:
        SISPOP_LOG(debug, "bad request");
        response_.result(http::status::bad_request);
        break;
    }
}
...
void connection_t::write_response() {
    ...
    const auto it = header_.find(SISPOP_EPHEMKEY_HEADER);
    ...
    if (!body_stream.empty()) {
        ...
        if (!response_.body().empty()) {
            SISPOP_LOG(debug, "Overwritting non-empty body in response!");
        }
    ...
    http::async_write(
        stream_, response_, [self = shared_from_this()](error_code ec, size_t) {
            if (ec && ec != boost::asio::error::operation_aborted) {
                SISPOP_LOG(error, "Failed to write to a socket: {}",
                         ec.message());
            }
...
void connection_t::process_store(const json& params) {
    ...
    for (const auto& field : fields) {
        if (!params.contains(field)) {
            ...
            SISPOP_LOG(debug, "Bad client request: no `{}` field", field);
            return;
        }
    }
    ...
    SISPOP_LOG(trace, "Storing message: {}", data);
    ...
    if (!created) {
        ...
        SISPOP_LOG(error, "Pubkey must be {} characters long", get_user_pubkey_size());
        return;
    }
    ...
    if (data.size() > MAX_MESSAGE_BODY) {
        ...
        body_stream_ << "Message body exceeds maximum allowed length of "
                     << MAX_MESSAGE_BODY << "\n";
        SISPOP_LOG(debug, "Message body too long: {}", data.size());
        return;
    }
...
#ifdef INTEGRATION_TEST
    SISPOP_LOG(trace, "store body: ", data);
#endif
    ...
    if (!util::parseTTL(ttl, ttlInt)) {
        ...
        SISPOP_LOG(debug, "Forbidden. Invalid TTL: {}", ttl);
        return;
    }
    ...
    if (!util::parseTimestamp(timestamp, ttlInt, timestampInt)) {
        ...
        SISPOP_LOG(debug, "Forbidden. Invalid Timestamp: {}", timestamp);
        return;
    }
    ...
    if (!success) {
        ...
        SISPOP_LOG(warn, "Service node is initializing");
        return;
    }
...
void connection_t::process_snodes_by_pk(const json& params) {
    ...
    if (!params.contains("pubKey")) {
        ...
        SISPOP_LOG(debug, "Bad client request: no `pubKey` field");
        return;
    }
    ...
    if (!success) {
        ...
        body_stream_ << fmt::format("Pubkey must be {} characters long\n",
                                    get_user_pubkey_size());
        SISPOP_LOG(debug, "Pubkey must be {} characters long ", get_user_pubkey_size());
        return;
    }
...
void connection_t::handle_wrong_swarm(const user_pubkey_t& pubKey) {
    ...
    /// This might throw if not utf-8 endoded
    body_stream_ << res_body.dump();
    SISPOP_LOG(debug, "Client request for different swarm received");
}
...
void connection_t::poll_db(const std::string& pk,
    ...
    const bool lp_requested = header_.find(SISPOP_LONG_POLL_HEADER) != header_.end();

    if (!items.empty()) {
        SISPOP_LOG(trace, "Successfully retrieved messages for {}",
                 obfuscate_pubkey(pk));
    }
    ...
    if (items.empty() && lp_requested) {
        ...
        notification_ctx_->timer.async_wait([=](const error_code& ec) {
            if (ec == boost::asio::error::operation_aborted) {
                SISPOP_LOG(trace, "Notification timer manually triggered");
                ...
                SISPOP_LOG(trace, "Notification timer expired");
...
void connection_t::process_client_req(const std::string& req_json) {
    ...
    if (body == nlohmann::detail::value_t::discarded) {
        response_.result(http::status::bad_request);
        body_stream_ << "invalid json\n";
        SISPOP_LOG(debug, "Bad client request: invalid json");
        return;
    }
    ...
    if (method_it == body.end() || !method_it->is_string()) {
        ...
        SISPOP_LOG(debug, "Bad client request: no method field");
        return;
    }
    ...
    if (params_it == body.end() || !params_it->is_object()) {
        ...
        SISPOP_LOG(debug, "Bad client request: no params field");
        return;
    }

    if (method_name == "store") {
        SISPOP_LOG(trace, "Process client request: store, connection: {}", this->conn_idx);
        this->process_store(*params_it);
    } else if (method_name == "retrieve") {
        SISPOP_LOG(trace, "Process client request: retrieve, connection: {}", this->conn_idx);
        this->process_retrieve(*params_it);
    } else if (method_name == "get_snodes_for_pubkey") {
        SISPOP_LOG(trace, "Process client request: snodes for pubkey");
        this->process_snodes_by_pk(*params_it);
    } else {
        response_.result(http::status::bad_request);
        body_stream_ << "no method" << method_name << "\n";
        SISPOP_LOG(debug, "Bad client request: unknown method '{}'", method_name);
    }
...
void connection_t::process_client_req_rate_limited() {
    ...
    if (rate_limiter_.should_rate_limit_client(client_ip)) {
        this->body_stream_ << "too many requests\n";
        response_.result(http::status::too_many_requests);
        SISPOP_LOG(debug, "Rate limiting client request.");
        return;
    }
    ...
    if (!parse_header(SISPOP_EPHEMKEY_HEADER)) {
        SISPOP_LOG(debug, "Bad client request: could not parse headers");
        return;
    }
    ...
    try {
        ...
        plain_text =
            channel_cipher_.decrypt(decoded, header_[SISPOP_EPHEMKEY_HEADER]);
    } catch (const std::exception& e) {
        response_.result(http::status::bad_request);
        response_.set(http::field::content_type, "text/plain");
        body_stream_ << "Could not decode/decrypt body: ";
        body_stream_ << e.what() << "\n";
        SISPOP_LOG(debug, "Bad client request: could not decrypt body");
        return;
    }
#endif
    ...
    if (req.find(SISPOP_LONG_POLL_HEADER) != req.end()) {
        header_[SISPOP_LONG_POLL_HEADER] = req.at(SISPOP_LONG_POLL_HEADER).to_string();
    }
...
void connection_t::register_deadline() {
    ...
    deadline_.async_wait([self = std::move(self)](error_code ec) {
        ...
        if (ec) {
            SISPOP_LOG(error, "Deadline timer error [{}]: {}", ec.value(),
                     ec.message());
        }

        SISPOP_LOG(debug, "Closing [connection_t] socket due to timeout");
        self->clean_up();
    });
}
...
void connection_t::on_shutdown(boost::system::error_code ec) {
    if (ec == boost::asio::error::eof) {
        ...
        SISPOP_LOG(debug, "Could not close ssl stream gracefully, ec: {} ({})",
                 ec.message(), ec.value());
    }

    const auto sockfd = stream_.lowest_layer().native_handle();
    SISPOP_LOG(trace, "Close https socket: {}", sockfd);
...
void connection_t::on_get_logs() {
    ...
    auto dev_sink = dynamic_cast<sispop::dev_sink_mt*>(
        spdlog::get("sispop_logger")->sinks()[2].get());

    if (dev_sink == nullptr) {
        SISPOP_LOG(critical, "Sink #3 should be dev sink");
        assert(false);
...
void HttpClientSession::on_connect() {
    ...
    SISPOP_LOG(trace, "Open http socket: {}", sockfd);
    get_net_stats().record_socket_open(sockfd);
...
void HttpClientSession::on_write(error_code ec, size_t bytes_transferred) {
    ...
    SISPOP_LOG(trace, "on write");
    if (ec) {
        SISPOP_LOG(error, "Http error on write, ec: {}. Message: {}", ec.value(),
                 ec.message());
        trigger_callback(SNodeError::ERROR_OTHER, nullptr);
        return;
    }

    SISPOP_LOG(trace, "Successfully transferred {} bytes", bytes_transferred);
...
void HttpClientSession::on_read(error_code ec, size_t bytes_transferred) {
    ...
    if (!ec || (ec == http::error::end_of_stream)) {
        ...
        SISPOP_LOG(trace, "Successfully received {} bytes.", bytes_transferred);
        ...
        if (http::to_status_class(res_.result_int()) ==
            ...
            SISPOP_LOG(error, "Http request failed, error code: {}",
                     res_.result_int());
        ...
        if (ec != boost::asio::error::operation_aborted) {
            SISPOP_LOG(error, "Error on read: {}. Message: {}", ec.value(),
                     ec.message());
        }
        trigger_callback(SNodeError::ERROR_OTHER, nullptr);
    }
    ...
    deadline_timer_.async_wait(
        [self = shared_from_this()](const error_code& ec) {
            if (ec) {
                ...
                SISPOP_LOG(debug, "client socket timed out");
                self->clean_up();
            }
        });
}
...
void HttpClientSession::trigger_callback(SNodeError error,
                                         std::shared_ptr<std::string>&& body) {
    SISPOP_LOG(trace, "Trigger callback");
...
void HttpClientSession::clean_up() {
    ...
    if (!needs_cleanup) {
        ...
        SISPOP_LOG(debug, "No need for cleanup");
        return;
    }
    ...
    if (!socket_.is_open()) {
        ...
        SISPOP_LOG(critical, "Socket is already closed");
        return;
    }
    ...
    if (ec && ec != boost::system::errc::not_connected) {
        SISPOP_LOG(error, "Socket shutdown failure [{}: {}]", ec.value(),
                 ec.message());
    }
    ...
    if (ec) {
        SISPOP_LOG(error, "Closing socket {} failed [{}: {}]", sockfd, ec.value(), ec.message());
    } else {
        SISPOP_LOG(trace, "Close http socket: {}", sockfd);
        get_net_stats().record_socket_close(sockfd);
    }
}
