...
void command_line_parser::parse_args(int argc, char* argv[]) {
    ...
    desc_.add_options()
        ...
        ("sispopd-rpc-ip", po::value(&options_.sispopd_rpc_port), "RPC IP on which the local Sispop daemon is listening (usually localhost)")
        ("sispopd-rpc-port", po::value(&options_.sispopd_rpc_port), "RPC port on which the local Sispop daemon is listening")
    ...
    hidden.add_options()
        ...
        ("sispopd-key", po::value(&options_.sispopd_key), "Legacy secret key (test only)")
        ("sispopd-x25519-key", po::value(&options_.sispopd_x25519_key), "x25519 secret key (test only)")
        ("sispopd-ed25519-key", po::value(&options_.sispopd_ed25519_key), "ed25519 public key (test only)");
    // clang-format on
    ...
    if (options_.testnet && !vm.count("sispopd-rpc-port")) {
        options_.sispopd_rpc_port = 38157;
    }
