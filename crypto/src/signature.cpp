...
extern "C" {
#include "sispop/crypto-ops/crypto-ops.h"
#include "sispop/crypto-ops/hash-ops.h"
}
...
static_assert(crypto_generichash_BYTES == sispop::HASH_SIZE, "Wrong hash size!");
...
signature generate_signature(const hash& prefix_hash,
                             const sispopd_key_pair_t& key_pair) {
    ge_p3 tmp3;
    ec_scalar k;
    s_comm buf;
    signature sig;
